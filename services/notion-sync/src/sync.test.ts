...
describe("mapTransaction", () => {
  it("maps a fully populated Balance Sheet page", () => {
    ...
    const page = makePage("tx-1", {
      Description: { type: "title", title: richText("Weekly groceries"), id: "p1" },
      Account: { type: "select", select: { id: "s1", name: "ANZ Everyday", color: "blue" }, id: "p2" },
      Amount: { type: "number", number: -125.50, id: "p3" },
      ...
      Location: { type: "select", select: { id: "s8", name: "Woolworths Bondi", color: "gray" }, id: "p8" },
      Country: { type: "select", select: { id: "s3", name: "Australia", color: "green" }, id: "p9" },
...
describe("mapInventoryItem", () => {
  it("maps a fully populated Home Inventory page", () => {
    ...
    const page = makePage("inv-1", {
      ...
      Type: { type: "select", select: { id: "s3", name: "Electronics", color: "purple" }, id: "p7" },
      Condition: { type: "select", select: { id: "s4", name: "Excellent", color: "green" }, id: "p8" },
...
describe("SQLite round-trip", () => {
  ...
  it("upserts and reads back entities with all fields", () => {
    ...
    const row = db.prepare("SELECT * FROM entities WHERE notion_id = ?").get("ent-1") as Record<string, unknown>;
  ...
  it("upserts transactions with notes field", () => {
    ...
    const row = db.prepare("SELECT * FROM transactions WHERE notion_id = ?").get("tx-1") as Record<string, unknown>;
  ...
  it("upserts inventory items with boolean conversion and relations", () => {
    ...
    const row = db.prepare("SELECT * FROM home_inventory WHERE notion_id = ?").get("inv-1") as Record<string, unknown>;
  ...
  it("upserts budget rows with boolean conversion", () => {
    ...
    const row = db.prepare("SELECT * FROM budgets WHERE notion_id = ?").get("bud-1") as Record<string, unknown>;
  ...
  it("upserts wish list rows", () => {
    ...
    const row = db.prepare("SELECT * FROM wish_list WHERE notion_id = ?").get("wl-1") as Record<string, unknown>;
