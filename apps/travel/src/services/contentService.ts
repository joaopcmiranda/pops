...
export class ContentService {
  ...
  static async searchContent(query: string): Promise<ContentItem[]> {
    try {
      ...
      const response = await client.get(`/trips/${tripId}/content/search?q=${encodeURIComponent(query)}`)
      return response.data || []
    } catch (error) {
  ...
  static async addContentItem(item: Omit<ContentItem, 'id' | 'createdAt' | 'updatedAt' | 'tripId' | 'userId'>): Promise<ContentItem | null> {
    try {
      const tripId = this.getCurrentTripId()
      const client = tripApiClient()
  ...
  static async updateContentItem(category: string, slug: string, updates: Partial<ContentItem>): Promise<boolean> {
    try {
      const tripId = this.getCurrentTripId()
      const client = tripApiClient()
