...
export const tripRouter = router({
  ...
  get: protectedProcedure.input(z.object({ id: z.string() })).query(async ({ ctx, input }) => {
    ...
    const itemsByType = itineraryItems.reduce(
      (acc, item) => {
    ...
    const itemsByStatus = itineraryItems.reduce(
      (acc, item) => {
      ...
      stats: {
        ...
        completionRate:
          itineraryItems.length > 0
            ? ((itemsByStatus.completed || 0) / itineraryItems.length) * 100
            : 0,
      },
    }
  }),
  ...
  create: protectedProcedure.input(createTripSchema).mutation(async ({ ctx, input }) => {
    ...
    const createData: TripCreateData = {
      ...Object.fromEntries(Object.entries(tripData).filter(([, value]) => value !== undefined)),
      title: input.title,
  ...
  templates: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const templates = await ctx.prisma.tripTemplate.findMany({
        where,
        orderBy: [{ usageCount: 'desc' }, { rating: 'desc' }, { createdAt: 'desc' }],
        take: input.limit || 20,
      })
  ...
  stats: protectedProcedure.query(async ({ ctx }) => {
    ...
    const upcomingTrips = trips.filter(
      trip => trip.startDate > currentDate && trip.status !== 'cancelled'
    ).length
