...
describe("corrections", () => {
  ...
  describe("createOrUpdate", () => {
    it("creates a new correction", async () => {
      const result = await caller.corrections.createOrUpdate({
    ...
    it("updates an existing correction on re-create, increasing confidence", async () => {
      await caller.corrections.createOrUpdate({
        descriptionPattern: "WOOLWORTHS",
        matchType: "contains",
        tags: ["Groceries"],
      });
  ...
  describe("findMatch", () => {
    ...
    it("finds exact match", async () => {
      const result = await caller.corrections.findMatch({
        description: "NETFLIX",
        minConfidence: 0,
      });
  ...
  describe("list", () => {
    it("lists all corrections", async () => {
      ...
      const result = await caller.corrections.list({});
    ...
    it("filters by minimum confidence", async () => {
      ...
      const highConfResult = await caller.corrections.list({ minConfidence: 0.9 });
      expect(highConfResult.data).toHaveLength(0);

      const lowConfResult = await caller.corrections.list({ minConfidence: 0.1 });
  ...
  describe("update", () => {
    it("updates tags on existing correction", async () => {
      const created = await caller.corrections.createOrUpdate({
        descriptionPattern: "UBER EATS",
        matchType: "contains",
        tags: ["Dining"],
      });

      const updated = await caller.corrections.update({
        id: created.data.id,
  ...
  describe("delete", () => {
    it("deletes a correction", async () => {
      ...
      await caller.corrections.delete({ id: created.data.id });
  ...
  describe("adjustConfidence", () => {
    it("increases confidence", async () => {
      ...
      await caller.corrections.adjustConfidence({ id: created.data.id, delta: 0.2 });

      const result = await caller.corrections.get({ id: created.data.id });
    ...
    it("deletes correction when confidence drops below 0.3", async () => {
      ...
      await caller.corrections.adjustConfidence({ id: created.data.id, delta: -0.3 });

      // Should be deleted since 0.5 - 0.3 = 0.2 < 0.3
      const list = await caller.corrections.list({});
