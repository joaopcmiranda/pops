...
import { createEnv, closeEnvDb, listEnvs } from "../modules/envs/registry.js";
...
// tRPC batch GET format: /trpc/transactions.list?batch=1&input={"0":{}}
// ---------------------------------------------------------------------------

async function listTransactions(envParam?: string) {
  const input = encodeURIComponent(JSON.stringify({ "0": {} }));
  let url = `/trpc/transactions.list?batch=1&input=${input}`;
