...
import { Plus, MapPin, Calendar, Users, Settings, Trash2 } from 'lucide-react'
...
export function TripSelector({ onTripSelect, onNewTrip }: TripSelectorProps) {
  ...
  // Fetch user's trips
  const tripsQuery = trpc.trip.list.useQuery({
    sortBy: 'startDate',
    sortOrder: 'desc',
    limit: 20,
  })
    ...
    return (
      <div className="trip-selector animate-fade-in">
        <div style={{ padding: '2rem', textAlign: 'center', color: '#ef4444' }}>
          Error loading trips: {tripsQuery.error.message}
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="trip-selector animate-fade-in" style={{ padding: '2rem', maxWidth: '1200px', margin: '0 auto' }}>
      {/* Header */}
      ...
      {trips.length === 0 ? (
        <Card>
          <CardContent style={{ padding: '3rem', textAlign: 'center' }}>
            <MapPin style={{ width: '64px', height: '64px', color: '#d1d5db', margin: '0 auto 1rem' }} />
            <h3 style={{ fontSize: '1.5rem', fontWeight: '600', color: '#0f172a', marginBottom: '0.5rem' }}>
              No trips yet
            </h3>
          ...
          {trips.map((trip: Trip) => (
            ...
            >
              <CardContent style={{ padding: '1.5rem' }}>
                ...
                {/* Trip Details */}
                <div style={{ display: 'flex', alignItems: 'center', color: '#64748b', fontSize: '0.875rem', marginBottom: '1rem' }}>
