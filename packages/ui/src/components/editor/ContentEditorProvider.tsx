...
import type { ContentEditorProviderProps, ContentEditorContextValue, ContentItem } from './types'
...
export const ContentEditorProvider: React.FC<ContentEditorProviderProps> = ({
  ...
  useEffect(() => {
    if (initialContent) {
      ...
      // Create new content item if no initial content provided
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      // Call external save handler - let the parent handle persistence
      if (onSave) {
  ...
  }, [state.content, state.isSaving, editor, onSave, onError])
